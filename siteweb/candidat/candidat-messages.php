...
<div class="dashboard-wrapper" style="height:100vh; overflow:hidden;">
  ...
  <!-- Dark Sidebar Left -->
  <?php require __DIR__ . '/../includes/candidat_sidebar.php'; ?>
  ...
  <div style="flex:1; display:flex; flex-direction:column; overflow:hidden;">
    ...
    <div class="dashboard-topbar" style="flex-shrink:0;">
      <div class="dashboard-header-left" style="display:flex; align-items:center; gap:1rem;">
        ...
        <p style="font-size:0.9rem; color:var(--text-muted);">Vos conversations avec les recruteurs</p>
      </div>
      </div>
      <div style="display:flex; align-items:center; gap:1.25rem;">
        <a href="../candidat/candidat-profil.php" title="Profil" style="display:flex;">
          <?php if (!empty($user['avatar']) && file_exists(__DIR__ . '/' . $user['avatar'])): ?>
            <img src="<?= htmlspecialchars($user['avatar']) ?>" alt="Avatar" style="width:42px; height:42px; border-radius:50%; object-fit:cover; border:2px solid #E2E8F0;">
          <?php else: ?>
        ...
        <div style="flex:1; overflow-y:auto;" id="conv-list">
          <?php if (empty($conversations)): ?>
            <div style="padding:3rem 1.5rem; text-align:center; color:#94A3B8;">
              ...
              <p style="font-size:0.8rem; margin-top:0.25rem;">Les recruteurs pourront vous contacter ici.</p>
            </div>
          <?php else: ?>
            <?php foreach ($conversations as $c):
              $isActive = ($c['id'] == $convId);
              $displayName = !empty($c['company_nom']) ? $c['company_nom'] : $c['contact_nom'];
            ...
            <a href="conv_id=<?= $c['id'] ?>" style="display:flex; align-items:center; gap:0.875rem; padding:1rem 1.25rem; border-bottom:1px solid #F8FAFC; text-decoration:none; background:<?= $isActive ? '#EFF6FF' : '#FFF' ?>; transition:background 0.15s;"
              ...
              <!-- Avatar -->
              <div style="width:46px; height:46px; border-radius:50%; background:<?= $bgColor ?>; display:flex; align-items:center; justify-content:center; font-weight:700; font-size:0.8rem; color:#FFB800; flex-shrink:0; position:relative;">
                <?php if (!empty($c['company_logo']) && file_exists(__DIR__ . '/' . $c['company_logo'])): ?>
                   <img src="<?= htmlspecialchars($c['company_logo']) ?>" alt="Logo" style="width:100%; height:100%; border-radius:50%; object-fit:cover;">
                <?php else: ?>
                   <?= $initials ?>
                <?php endif; ?>
      ...
      <div class="chat-active-panel" style="flex:1; display:flex; flex-direction:column; overflow:hidden; background:#FFF;">
        ...
        <?php if ($activeConv): ?>
          ...
          <div style="padding:1rem 1.5rem; border-bottom:1px solid #E2E8F0; display:flex; align-items:center; justify-content:space-between; flex-shrink:0;">
            <div style="display:flex; align-items:center; gap:0.875rem;">
              <a href="candidat-messages.php" class="chat-back-btn" style="color: #64748B; background: none; border: none; cursor: pointer; align-items: center; justify-content: center; text-decoration: none;">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg>
              </a>
              <?php
                $dn = !empty($activeConv['company_nom']) ? $activeConv['company_nom'] : $activeConv['contact_nom'];
                $ini = strtoupper(substr($dn, 0, 2));
                $bgColors = ['#081326','#DC2626','#0284C7','#EA580C','#7C3AED','#059669'];
                $bg = $bgColors[$activeConv['id'] % count($bgColors)];
              ?>
              <div style="width:42px; height:42px; border-radius:50%; background:<?= $bg ?>; display:flex; align-items:center; justify-content:center; font-weight:700; font-size:0.8rem; color:#FFB800; position:relative; flex-shrink:0;">
                <?php if (!empty($activeConv['company_logo']) && file_exists(__DIR__ . '/' . $activeConv['company_logo'])): ?>
                   <img src="<?= htmlspecialchars($activeConv['company_logo']) ?>" alt="Logo" style="width:100%; height:100%; border-radius:50%; object-fit:cover;">
                <?php else: ?>
                   <?= $ini ?>
                <?php endif; ?>
                <span style="position:absolute; bottom:1px; right:1px; width:11px; height:11px; background:#22C55E; border-radius:50%; border:2px solid #FFF;"></span>
          ...
          <div id="chat-scroll" style="flex:1; overflow-y:auto; padding:1.5rem; display:flex; flex-direction:column; gap:1rem; background:#F8FAFC;">
            <?php if (empty($messages)): ?>
              <div style="text-align:center; color:#94A3B8; margin-top:3rem;">
                ...
                <p style="font-size:0.8rem;">L'entreprise devrait vous contacter bientôt !</p>
              </div>
            <?php else: ?>
              <?php
              $prevDate = '';
              ...
              <div style="display:flex; justify-content:<?= $isMe ? 'flex-end' : 'flex-start' ?>; align-items:flex-end; gap:0.5rem;">
                <?php if (!$isMe): ?>
                  <div style="width:32px; height:32px; border-radius:50%; background:<?= $bg ?>; display:flex; align-items:center; justify-content:center; font-size:0.65rem; font-weight:700; color:#FFB800; flex-shrink:0; overflow:hidden;">
                    <?php if (!empty($activeConv['company_logo']) && file_exists(__DIR__ . '/' . $activeConv['company_logo'])): ?>
                       <img src="<?= htmlspecialchars($activeConv['company_logo']) ?>" alt="Logo" style="width:100%; height:100%; object-fit:cover;">
                    <?php else: ?>
                       <?= $ini ?>
                    <?php endif; ?>
                  </div>
        ...
        <?php else: ?>
          ...
          <div style="flex:1; display:flex; flex-direction:column; align-items:center; justify-content:center; color:#94A3B8; gap:1rem;">
            ...
            <p style="font-size:0.875rem;">Vos échanges avec les recruteurs apparaîtront ici.</p>
          </div>
        <?php endif; ?>
  ...
  function appendMessage(msg, isMe) {
    ...
    if (!isMe) {
      html += `
        <div style="width:32px; height:32px; border-radius:50%; background:<?= $bg ?>; display:flex; align-items:center; justify-content:center; font-size:0.65rem; font-weight:700; color:#FFB800; flex-shrink:0; overflow:hidden;">
          <?php if (!empty($activeConv['company_logo']) && file_exists(__DIR__ . '/' . $activeConv['company_logo'])): ?>
             <img src="<?= htmlspecialchars($activeConv['company_logo']) ?>" style="width:100%; height:100%; object-fit:cover;">
          <?php else: ?>
             <?= $ini ?>
          <?php endif; ?>
        </div>
